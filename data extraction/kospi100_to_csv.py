...
class CMarketTotal():
    ...
    def PrintMarketTotal(self):
        ...
        f = open("kospi100.csv", "w", encoding="utf-8-sig")
        for item in data2[:100]:    #상위 100개만 뽑기
        #for item in data2:
            Vcode = item[0]
            name = g_objCodeMgr.CodeToName(item[0])
            listed = item[1][0]
            markettot = item[1][1]
            #print('%s, %s, %s, %s' %(Vcode, name, format(listed, ','), format(markettot, ',')))
            f.write(Vcode + ',' + name + ',' + str(listed) + ',' + str(markettot) +'\n')
        f.close() 
